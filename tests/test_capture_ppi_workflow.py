...
class PpiWorkflowTests(unittest.TestCase):
 def test_capture_workflow_contract(self):
  ...
  for value in ('name: Capture PPI Release','workflow_dispatch:','schedule:','run_due_ppi_capture.py','contents: write','concurrency:','actions/checkout@v6','actions/setup-python@v6','actions/upload-artifact@v4','data/releases/ppi/{event_id}/as_released.json') : self.assertIn(value,text)
