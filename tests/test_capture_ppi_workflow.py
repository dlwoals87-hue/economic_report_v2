...
class PpiWorkflowTests(unittest.TestCase):
 def test_capture_workflow_contract(self):
  ...
  for value in ('git add -- "$COMMIT_PATH"','git diff --cached --name-only > "$STAGED_PATHS_FILE"','git diff --cached --name-status > "$STAGED_STATUS_FILE"','if len(staged) != 1 or len(set(staged)) != 1:','if staged != [expected]:','if len(statuses) != 1 or statuses[0] != ["A", expected]:','if not path.is_file() or path.is_symlink():','len(pure.parts) != 5','staged paths do not match commit path','PPI release must be staged as a new file') : self.assertIn(value,text)
